...
set -- \
    ...
    --schema-function-corpus "$source_root/johin/function-calling.jsonl" --seed "$seed" --profile v11
...
"$repo_dir/target/release/calib_compose" --model-dir "$model_dir" --output "$output_jsonl" \
    ...
    --reserve-sequences "$reserve_sequences" --trajectory-packing \
    --source "general_long_multiturn=10:2048:$staging_dir/general_long_multiturn.jsonl" \
    --source "code=16:1024:$staging_dir/code.jsonl" \
    --source "multilingual=14:768:$staging_dir/multilingual.jsonl" \
    --source "tools_structured=17:2048:$staging_dir/tools_structured.jsonl" \
    --source "agentic_reliability=10:2048:$staging_dir/agentic_reliability.jsonl" \
    --source "workflow_reliability=20:4096:$staging_dir/workflow_reliability.jsonl" \
    --source "schema_function=8:1024:$staging_dir/schema_function.jsonl" \
    --source "math_reasoning=3:2048:$staging_dir/math_reasoning.jsonl" \
    --source "prompt_injection=2:768:$staging_dir/prompt_injection.jsonl"

"$repo_dir/target/release/calib_compose" --model-dir "$model_dir" --output "$long_output" \
    --nsamples 64 --seqlen 8192 --trajectory-packing \
    --source "general_long_context=75:8192:$staging_dir/general_long_context.jsonl" \
    --source "workflow_reliability=25:4096:$staging_dir/workflow_reliability.jsonl"

echo "[generate-public-v11] main: $output_jsonl"
echo "[generate-public-v11] long: $long_output"
